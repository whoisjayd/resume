...
help:
	@$(UV) run python -c "print('\n'.join(['', '  RESUME TOOLKIT', '  Build and maintain the RenderCV resume from one consistent interface.', '', '  USAGE', '    make <command> [VARIABLE=value]', '', '  DEVELOPMENT', '    setup       Synchronize the Python environment from uv.lock', '    watch       Re-render continuously while editing YAML sources', '    check       Validate sources with a quiet, complete render', '    doctor      Display tool versions and active configuration', '', '  BUILD', '    render      Generate all formats into local resume/', '    private     Render local resume/ with optional .env phone', '    public      Render sanitized formats into tracked public/', '    pdf         Generate only the local resume.pdf', '    preview     Generate only local PNG previews', '    rebuild     Remove existing local artifacts, then generate everything', '', '  MAINTENANCE', '    clean       Remove generated artifacts from the output directory', '    help        Display this command reference (default)', '', '  CONFIGURATION', '    CV=$(CV)', '    DESIGN=$(DESIGN)', '    LOCALE=$(LOCALE)', '    SETTINGS=$(SETTINGS)', '    OUTPUT_DIR=$(OUTPUT_DIR)', '', '  OUTPUTS', '    All render targets clear stale files and normalize a single page to resume.png.', '    Multi-page renders retain numbered resume_*.png files.', '', '  EXAMPLES', '    make render', '    make private', '    make public', '    make render CV=alternate-cv.yaml OUTPUT_DIR=build', '']))"
...
public: render ## Generate the sanitized public site in public/
	@$(UV) run python -c "from pathlib import Path; source=Path('public/resume.html'); Path('public/index.html').write_bytes(source.read_bytes())"
